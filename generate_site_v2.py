# -*- coding: utf-8 -*-
"""
Site Portfolio Salah Barki - V2 Propre
- 6 pages statiques d'intervention (plus de bug JS)
- Suppression logout / user / account
- Design Industrial Precision
"""
import os

OUTPUT_DIR = r"C:\Users\ba2rb\Downloads\salpre\site_v2"
...
FICHES = [
    ...
    },
    {
        ...
        "diagnostic": "Connexion sur TIA Portal en ligne avec l'automate. Monitoring des blocs de programme.<br><br>Le temps d'exécution du bloc de gestion robot est anormalement long (8 secondes mesurées sur l'outil de diagnostic intégré, contre 3–4 s attendu).<br><br>Analyse pas à pas du programme : une temporisation de sécurité est réglée à 8 secondes alors que le mouvement réel du bras robot dure environ 5,5 s (mesuré au chronomètre et confirmé par le suivi des signaux d'entrées/sorties). La temporisation est donc trop conservative et bloque inutilement la suite du cycle.<br><br>Bit de validation 'pince OK' : capteur de proximité qui détecte la pièce clignote légèrement au lieu d'être fixe. Inspection mécanique : le support du capteur est légèrement desserré (jeu de 2 mm). Le capteur détecte parfois, parfois pas => le programme attend indéfiniment le signal stable.<br><br>Programmation non optimisée : les mouvements du bras sont strictement séquentiels (un à la fois) alors que certains mouvements compatibles pourraient être faits en parallèle (ex: monter le bras tout en le déplaçant latéralement, avec sécurité de hauteur).",
        "actions": [
    ...
    },
    {
        ...
        "diagnostic": "Analyse des données de test des 15 derniers jours (extraites du système informatique du banc) : dérive lente et progressive de la résistance mesurée au test de continuité. Tolérance : <10 mΩ. Valeurs de départ 3–5 mΩ → dérive progressive 8–12 mΩ → dépassement du seuil = rebut.<br><br>Pareto des causes de rebut sur 342 pièces rebutées : ~2/3 liés aux broches de contact du connecteur adaptateur du banc (pins), 1/4 liés aux câbles de piquage usés, le reste divers.<br><br>Inspection visuelle des pins : 14 des 32 pins présentent une oxydation noire visible (couche de CuO). Cause probable : absence de nettoyage régulier des pins dans la gamme de maintenance (le dernier nettoyage n'était pas documenté, probablement >6 mois).<br><br>Hotte d'extraction des fumées de soudure au poste amont : filtre saturé, débit d'aspiration faible (mesuré au débitmètre : 80 m³/h au lieu des ~120 m³/h nécessaires). Les fumées de soudure contenant des résidus de flux dérivent vers le banc de test et contaminent progressivement les contacts.<br><br>Mesure au micro-ohmètre : pins oxydés = 15–45 mΩ. Pins nettoyés à la brosse + alcool = <2 mΩ.",
        "actions": [
    ...
    },
    {
        ...
        "diagnostic": "Mesures aux manomètres multi-points :<br>- Pression côté pompe (amont filtre) : 80 bar stable.<br>- Pression côté descente (amont vérin tige) : 32 bar (anormal, devrait être ~78–80 bar).<br>- Pression côté montée : 78 bar (normal).<br>→ Panne localisée côté circuit descente.<br><br>Auscultation au stéthoscope mécanique : bruit de coup de bélier localisé au niveau de l'accumulateur hydropneumatique côté montée. Fréquence synchrone avec l'inversion du cycle.<br><br>Démontage du clapet anti-retour sur la ligne de descente : présence d'un petit morceau de joint torique écrasé coincé dans le siège du clapet (corps étranger). Le joint du clapet est partiellement dégradé, probablement à cause de la température de l'huile plus élevée que la normale.<br><br>Température huile mesurée au thermomètre de cuve : 68 °C (normalement <55 °C en exploitation régulière). Cause surchauffe : le filtre de retour est très encrassé (noir et plein de limaille) => la pompe force dans un circuit bouché => surchauffe + dégradation des joints.<br><br>Vérification de l'accumulateur hydropneumatique : précharge d'azote mesurée au manomètre de contrôle = 12 bar (normalement ~65 bar pour ce modèle). La membrane interne est visiblement fissurée (trace d'huile dans la partie gaz lors du démontage de contrôle).",
        "actions": [
    ...
    },
    {
        ...
        "diagnostic": "Analyse des données de la GMAO et de la supervision des 12 derniers mois :<br>- 342 ordres de travail préventifs planifiés, 233 réalisés (68%), 109 reportés.<br>- Pareto des causes de report : ~45% = conflit avec production (pas de fenêtre machine disponible), ~30% = pièces détachées non disponibles (délai d'approvisionnement 5–7 jours), ~15% = sous-traitant spécialisé indisponible.<br><br>Absence de classification des équipements par criticité : tous traités avec la même priorité. Exemple : le compresseur d'air 8 bar (critique, arrêt = arrêt tout le site) et un éclairage de bureau avaient la même fréquence de préventif. Le temps de maintenance était mal réparti.<br><br>Analyse MTBF par famille d'équipements : presses injection 380 h (faible), bancs test 620 h (moyen), utilités 850 h (bon). Aucun plan d'action spécifique sur les presses.",
        "actions": [
...
# Top nav SANS user/account/settings icons
TOP_NAV = '''
<header class="bg-slate-50 dark:bg-slate-950 font-sans tracking-tight uppercase font-bold text-xs docked full-width top-0 border-b border-slate-300 dark:border-slate-800 flex justify-between items-center w-full px-10 h-16 fixed z-50 pl-64">
    <div class="flex items-center gap-6">
        ...
        <nav class="hidden md:flex items-center gap-6 ml-8">
            <a class="{journal_active}" href="journal.html">Journal</a>
            <a class="{cert_active}" href="index.html#certifications">Certifications</a>
            <a class="{cv_active}" href="contact.html">CV & Contact</a>
            <a class="{home_active}" href="index.html">Accueil</a>
        </nav>
    </div>
</header>
'''

# Side nav SANS profile image/ID/logout
SIDE_NAV = '''
<aside class="bg-slate-100 dark:bg-slate-900 font-sans text-sm font-medium tracking-wide fixed left-0 top-0 h-screen border-r w-64 border-slate-300 dark:border-slate-800 flex flex-col py-6 z-40 pt-20">
    ...
    <nav class="flex flex-col flex-grow">
        ...
        <a class="text-slate-600 dark:text-slate-400 px-4 py-3 hover:bg-slate-200 dark:hover:bg-slate-800 flex items-center gap-3" href="index.html#certifications"><span class="material-symbols-outlined">verified</span> Certifications</a>
        <a class="text-slate-600 dark:text-slate-400 px-4 py-3 hover:bg-slate-200 dark:hover:bg-slate-800 flex items-center gap-3" href="contact.html"><span class="material-symbols-outlined">contact_mail</span> Contact</a>
    </nav>
    <div class="px-4 py-4">
...
<footer class="bg-slate-50 dark:bg-slate-950 font-sans text-[10px] uppercase tracking-tighter text-slate-500 w-full border-t border-slate-300 dark:border-slate-800 py-8 px-10 flex flex-col md:flex-row justify-between items-center gap-4 ml-64 mt-auto">
    ...
    <div class="flex items-center gap-4">
        <a class="text-slate-500 hover:text-orange-600 dark:hover:text-orange-400 transition-colors" href="index.html">Accueil</a>
        <a class="text-slate-500 hover:text-orange-600 dark:hover:text-orange-400 transition-colors" href="journal.html">Journal</a>
        <a class="text-slate-500 hover:text-orange-600 dark:hover:text-orange-400 transition-colors" href="contact.html">Contact</a>
    </div>
</footer>
'''

NAV_ACTIVE = "text-sky-900 dark:text-sky-400 border-b-2 border-sky-900 dark:border-sky-400 pb-1 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors opacity-80 duration-75"
NAV_INACTIVE = "text-slate-500 dark:text-slate-400 hover:text-sky-800 dark:hover:text-sky-300 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors"

SIDE_ACTIVE = "bg-sky-900 text-white dark:bg-sky-400 dark:text-slate-950 rounded-none border-l-4 border-orange-500 font-bold scale-95 duration-100"
SIDE_INACTIVE = "text-slate-600 dark:text-slate-400"


def make_nav(active_page):
    # active_page: 'index', 'journal', 'contact'
    ja = NAV_ACTIVE if active_page == 'journal' else NAV_INACTIVE
    ca = NAV_ACTIVE if active_page == 'index' else NAV_INACTIVE
    va = NAV_ACTIVE if active_page == 'contact' else NAV_INACTIVE
    ha = NAV_ACTIVE if active_page == 'index' else NAV_INACTIVE
    nav = TOP_NAV.format(journal_active=ja, cert_active=ca, cv_active=va, home_active=ha)
    
    oa = SIDE_ACTIVE if active_page == 'index' else SIDE_INACTIVE
    ia = SIDE_ACTIVE if active_page == 'journal' else SIDE_INACTIVE
    side = SIDE_NAV.format(name=PROFILE["name"], title=PROFILE["title"], overview_active=oa, interventions_active=ia)
    return nav, side
...
def build_journal():
    ...
    for f in FICHES:
        ...
        short_symptome = f['symptome'][:150] + "..." if len(f['symptome']) > 150 else f['symptome']
        cards += f'''
                ...
                <div class="p-6 flex flex-col flex-grow">
                    <header class="flex justify-between items-start mb-stack-sm">
                        <div class="flex items-center gap-2">
                            <span class="material-symbols-outlined text-secondary text-[20px] {filled}">{icon}</span>
                    ...
                    <p class="font-body-md text-body-md text-on-surface mb-stack-md flex-grow">{short_symptome}</p>
                    <div class="mt-auto border-t border-outline-variant pt-4 flex justify-between items-center">
                        <span class="font-data-mono text-data-mono text-on-surface-variant text-sm">{f["entreprise"]} — {f["lieu"]}</span>
                        <a href="intervention_{f["num"]}.html" class="font-label-caps text-label-caps text-primary hover:text-secondary flex items-center gap-1 transition-colors">
                ...
                <div class="bg-surface-container-low border border-outline-variant p-4 rounded-DEFAULT">
                    ...
                    <div class="space-y-stack-sm">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <div class="w-4 h-4 border border-outline bg-surface group-hover:border-primary flex items-center justify-center">
                                <span class="material-symbols-outlined text-[12px] text-primary opacity-0 group-hover:opacity-100">check</span>
                            </div>
                            <span class="font-data-mono text-data-mono text-on-surface">Hydraulics</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <div class="w-4 h-4 border border-primary bg-primary flex items-center justify-center">
...
# ============================================================
# PAGE INTERVENTION STATIQUE
# ============================================================
def build_intervention(f):
    nav, side = make_nav('journal')
    
    actions_html = "\n".join([
        f'''<div class="flex gap-3 border-b border-outline-variant pb-stack-sm">
            <span class="font-data-mono text-data-mono text-secondary shrink-0">{str(i+1).zfill(2)}.</span>
            <p class="font-body-md text-body-md text-on-surface">{action}</p>
        </div>''' for i, action in enumerate(f["actions"])
    ])
    
    metriques_html = "\n".join([
        f'''<div class="bg-surface p-stack-sm border border-outline flex flex-col justify-center items-center text-center">
            <span class="material-symbols-outlined text-primary mb-1">{m["icon"]}</span>
            <div class="font-label-caps text-label-caps text-on-surface-variant mb-unit">{m["label"]}</div>
            <div class="font-h3 text-h3 text-primary">{m["value"]}</div>
        </div>''' for m in f["metriques"]
    ])
    
    pieces_html = "\n".join([
        f'''<div class="bg-surface-container-lowest border border-outline p-stack-sm flex items-start gap-3">
            <div class="bg-surface p-1.5 border border-outline-variant mt-0.5">
                <span class="material-symbols-outlined text-on-surface-variant text-[18px]">settings</span>
            </div>
            <div>
                <div class="font-body-md text-body-md font-bold text-on-surface leading-tight mb-1">{p["name"]}</div>
                <div class="flex justify-between items-center w-full gap-4">
                    <span class="font-data-mono text-data-mono text-on-surface-variant text-sm">Réf: {p["ref"]}</span>
                    <span class="font-data-mono text-data-mono text-on-surface text-sm">Qté: {p["qty"]}</span>
                </div>
            </div>
        </div>''' for p in f["pieces"]
    ]) if f["pieces"] else '<div class="font-data-mono text-data-mono text-on-surface-variant">Aucune pièce remplacée pour cette intervention.</div>'
    
    outillage_html = "\n".join([
        f'''<div class="border border-outline bg-surface px-2 py-1 flex items-center gap-1">
            <span class="material-symbols-outlined text-[14px] text-primary">build</span>
            <span class="font-label-caps text-label-caps text-primary">{o}</span>
        </div>''' for o in f["outillage"]
    ])
...
<body class="bg-surface text-on-surface font-body-md text-body-md antialiased min-h-screen flex flex-col">
    ...
    <main class="ml-64 mt-16 flex-grow p-margin bg-surface">
        ...
        <!-- Page Header -->
        <div class="border-b border-outline pb-stack-md mb-stack-lg flex flex-col md:flex-row md:justify-between md:items-end gap-stack-md">
            <div>
                <div class="font-label-caps text-label-caps text-secondary mb-stack-sm flex items-center gap-2">
                    <span class="w-2 h-2 bg-secondary block"></span> {f["type"]}
                </div>
                <h1 class="font-h1 text-h1 text-primary">{f["titre"]}</h1>
                <p class="font-body-lg text-body-lg text-on-surface-variant mt-unit">Rapport d'intervention technique #{f["id"]}</p>
            </div>
            <div class="flex flex-col md:text-right gap-unit">
                <div class="font-data-mono text-data-mono text-on-surface-variant flex items-center md:justify-end gap-2">
                    <span class="material-symbols-outlined text-[16px]">calendar_today</span> {f["date"]}
                </div>
                <div class="font-data-mono text-data-mono text-on-surface-variant flex items-center md:justify-end gap-2">
                    <span class="material-symbols-outlined text-[16px]">timer</span> Durée: {f["duree"]}
                </div>
                <div class="font-data-mono text-data-mono text-on-surface-variant flex items-center md:justify-end gap-2">
                    <span class="material-symbols-outlined text-[16px]">engineering</span> Tech: {f["tech"]}
                </div>
            </div>
        </div>
        <!-- 12-Col Grid -->
            ...
            <div class="lg:col-span-8 space-y-gutter">
                ...
                <section class="bg-surface-container-lowest border border-outline rounded relative overflow-hidden">
                    ...
                    <div class="p-gutter">
                        ...
                        <p class="font-body-md text-body-md text-on-surface mb-stack-md">{f["symptome"]}</p>
                        <div class="bg-surface p-stack-md border border-outline flex gap-4 items-center">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex justify-between items-center border-b border-outline-variant pb-1">
                                    ...
                                    <span class="font-data-mono text-data-mono text-error font-bold">{f["equip"][:35]}...</span>
                                </div>
                                <div class="flex justify-between items-center pt-1">
                                    <span class="font-data-mono text-data-mono text-on-surface-variant text-sm">Site</span>
                                    <span class="font-data-mono text-data-mono text-on-surface-variant text-sm">{f["lieu"]}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section class="bg-surface-container-lowest border border-outline rounded">
                    <div class="p-gutter">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md mb-stack-md">
                            <div>
                                <p class="font-body-md text-body-md text-on-surface">{f["diagnostic"]}</p>
                            </div>
                ...
                <section class="bg-surface-container-lowest border border-outline rounded relative overflow-hidden">
                    ...
                    <div class="p-gutter">
                        ...
                        <p class="font-body-md text-body-md text-on-surface mb-stack-md">{f["resultat"]}</p>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-unit">
                            {metriques_html}
                        </div>
                    </div>
                </section>
                ...
                <div class="bg-surface-container border border-outline rounded p-gutter">
                    ...
                    <ul class="space-y-stack-sm">
                        <li class="flex justify-between border-b border-outline-variant pb-unit">
                            ...
                            <span class="font-data-mono text-data-mono text-on-surface">{f["equip"][:40]}...</span>
                        </li>
                        <li class="flex justify-between border-b border-outline-variant pb-unit">
                            <span class="font-body-md text-body-md text-on-surface-variant">Localisation</span>
                            <span class="font-data-mono text-data-mono text-on-surface">{f["lieu"]}</span>
                        </li>
                        <li class="flex justify-between border-b border-outline-variant pb-unit">
                            <span class="font-body-md text-body-md text-on-surface-variant">Entreprise</span>
                            <span class="font-data-mono text-data-mono text-on-surface">{f["entreprise"]}</span>
                        </li>
...
'''
    return COMMON_HEAD.format(title=f"{f['titre']} — TechMaintenance Pro") + body + "</html>"
...
<body class="bg-background text-on-background min-h-screen flex flex-col font-body-md">
    ...
    <main class="flex-grow w-full max-w-container-max mx-auto px-margin py-stack-lg flex flex-col md:flex-row gap-gutter ml-64 mt-16">
        ...
        <div class="w-full md:w-1/3 flex flex-col gap-stack-lg">
            <div>
                <h1 class="font-h1 text-h1 text-primary mb-stack-sm">Contact</h1>
            ...
            <div class="bg-surface-container-low border border-outline-variant p-stack-md rounded-DEFAULT flex flex-col items-center justify-center text-center gap-stack-sm relative overflow-hidden group">
                ...
                <a href="../Portfolio_Salah_Barki_v5.pptx" download class="bg-primary text-on-primary font-label-caps text-label-caps uppercase px-6 py-3 rounded-DEFAULT hover:bg-surface-tint transition-colors w-full flex items-center justify-center gap-2">
        ...
        <div class="w-full md:w-2/3">
            <div class="bg-surface border border-outline-variant rounded-DEFAULT p-stack-lg h-full">
                <div class="flex items-center gap-2 mb-stack-md border-b border-outline-variant pb-stack-sm">
                    ...
                    <h2 class="font-h2 text-h2 text-primary">Envoyer un message</h2>
                </div>
                <form class="flex flex-col gap-stack-md" onsubmit="event.preventDefault(); alert('Message envoyé !');">
...
if __name__ == "__main__":
    ...
    write_file("journal.html", build_journal())
    write_file("contact.html", build_contact())
    for f in FICHES:
        write_file(f"intervention_{f['num']}.html", build_intervention(f))
    print(f"\n=== SITE V2 GÉNÉRÉ DANS {OUTPUT_DIR} ===")
